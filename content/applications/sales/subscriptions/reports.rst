...
Saving filters and groupings
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If desired, a filter or grouping (or combination of the two) can be saved in the
:guilabel:`Favorites` section of the drop-down search menu. To do so, click the
:icon:`fa-caret-down` icon beside :guilabel:`Save current search`, located beneath the
:icon:`fa-star` :guilabel:`Favorites` section. This reveals a field to assign a title to the
filter/grouping. Ticking the checkbox beside :guilabel:`Default filter` makes the newly-favorited
filter the default option for that reporting page.

To save the filter, click :guilabel:`Save` in the :guilabel:`Favorites` section of the drop-down
filter menu. The saved filter then appears beneath the :guilabel:`Favorites` column of the drop-down
filter menu.
...
- :guilabel:`Recurring Revenue`: Measures the gross recurring revenue for the subscriptions based
  off of a non-monthly recurring plan.
...
- :guilabel:`Untaxed Total`: Measures the total cost of the products in the subscription that aren't
  subject to a tax.
- :guilabel:`Yearly Recurring`: Measures the projected yearly revenue for the subscriptions, even if
  they aren't on a yearly recurring plan.
...
- :guilabel:`Amount To Pay In POS`: Measures amount to be paid in still-to-be-invoiced
  subscriptions.
