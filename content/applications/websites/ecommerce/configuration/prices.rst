...
Odoo offers multiple options for configuring prices in e-commerce. For example, you can apply
...
(:doc:`B2B vs. B2C <b2b_b2c>`). To select the type of price displayed, go to :menuselection:`Website
--> Configuration --> Settings`, select the website, scroll down to the :guilabel:`eCommerce`
...
#. Enable the :guilabel:`Product Reference Price` checkbox in the :guilabel:`eCommerce` section.
...
Pricelists in Odoo allow for managing flexible pricing rules and offering different prices and
...
.. tip::
   - If you are using several websites and want to use the same pricelist on several of them, you
     need to duplicate the pricelist for each website. If you do not assign any website, the
...
<../../../sales/sales/products_prices/prices/currencies>`, and allow customers to choose a pricelist
using the pricelist selector in the catalog, located next to the :ref:`search bar
...
.. note::
   - If a pricelist is designated as :guilabel:`Selectable` and not assigned to a specific website,
     it is selectable on **all** websites.
...
.. important::
   - When using GeoIP to determine the appropriate pricelist, **all** pricelists used on the
     e-commerce site must have a country group assigned.
   - It is strongly recommended to disable the :guilabel:`Selectable` option when using pricelists
     ...
     .. example::
        ...
        and prices will be displayed based on the `EU regular prices` pricelist in the catalog.
...
.. example::
   ...
   assignment, so the :guilabel:`Loyal Customer Discount` is applied.
...
When the price of a specific product is permanently reduced, the strikethrough strategy is commonly
used. This involves showing the new discounted price, with the original price crossed out alongside
it.
...
Settings`, scroll down to the :guilabel:`eCommerce` section, enable the :guilabel:`Comparison Price`
option, and :guilabel:`Save`. Then :ref:`go to the product form
<ecommerce/products/product-creation>`, enter the new (discounted) price in the :guilabel:`Sales
Price` field, and the original price (strikethrough) in the :guilabel:`Compare to Price` field.
...
.. note::
  ...
  must be assigned the same country group as the non-discounted pricelist.
...
Then, go to the :guilabel:`Rules` tab, click :guilabel:`Add a line`, and :ref:`configure the
discount <sales/products/price-rules>`. When a customer enters the code during :ref:`checkout
<ecommerce/checkout/review_order>`, the discount is applied according to the configuration in the
:guilabel:`Rules` tab.
...
- For :guilabel:`Specific categories`: If you select this option, the :guilabel:`Categories` submenu
  appears. Select **at least one** product :ref:`category
  <ecommerce/categories_variants/categories>` from the drop-down menu. Prices for these categories
  are only displayed if the product price is greater than `0`.

Next, enter a redirect URL in the :guilabel:`Link URL` field. The :guilabel:`Add to Cart` button
