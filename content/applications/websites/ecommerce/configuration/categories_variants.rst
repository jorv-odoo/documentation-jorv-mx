...
#. Scroll down to the :guilabel:`eCommerce` section and enable the :guilabel:`Product Variants`
   feature.
...
Once your attributes are :ref:`created <products/variants/attributes>`, navigate to the
:guilabel:`eCommerce` tab of the attribute form and configure the following options to control
where and/or how they appear on the website:

- :guilabel:`Filter`: Set the field to :guilabel:`Visible` to make the attribute available in the
  shop's filter section.
- :guilabel:`On Product Cards`: Set the field to :guilabel:`Visible` or :guilabel:`Hover` to display
  ...
  :ref:`variants are created instantly for the attribute <products/variants/attributes>`. When
  :guilabel:`On Product Cards` is enabled, the :guilabel:`Show Thumbnails` option can be toggled to
  show a product variant image instead of the attribute values.

.. tip::
   Access all attributes by going to :menuselection:`Website --> eCommerce --> Attributes`.
...
:guilabel:`Category` in the :guilabel:`eCommerce` tab of the attribute form or create a new one.
Click the :icon:`fa-arrow-right` :guilabel:`(internal link)` icon next to the category name to:
...
E-commerce-specific categories are used to organize products into groups, making it easier for
customers to browse the online store.
...
- In the :guilabel:`Description` tab:
  ...
  - Write a category :guilabel:`Description`.
...
.. tip::
   - You can also assign categories directly from the :ref:`product form
